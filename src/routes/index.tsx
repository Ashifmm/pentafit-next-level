...
import story2 from "@/assets/uploads/story-2.jpg.asset.json";
import story4 from "@/assets/uploads/story-4.jpg.asset.json";
const heroImg = heroAsset.url;
const coach = heroAsset.url;
const t1 = story1.url;
const t2 = story4.url;
const t3 = story2.url;
...
function Pricing() {
  const plans = [
    { name: "Starter", price: "₹4,999", period: "/month", popular: false, features: ["Personalized diet plan", "Workout template", "App access", "Weekly check-in"] },
    { name: "Pro", price: "₹8,999", period: "/month", popular: true, features: ["1:1 coach assignment", "Custom workouts (gym/home)", "Adaptive nutrition", "Bi-weekly video reviews", "WhatsApp support"] },
    { name: "Elite", price: "₹14,999", period: "/month", popular: false, features: ["Everything in Pro", "Daily coach access", "Lab marker review", "Habit & mindset coaching", "Priority response"] },
  ];
  return (
    <section id="pricing" className="container-prose py-24">
      <Reveal className="text-center max-w-2xl mx-auto">
        <p className="text-xs font-bold uppercase tracking-[0.2em] text-brand-dark">Simple, premium pricing</p>
        <h2 className="mt-3 text-3xl md:text-5xl font-extrabold font-display tracking-tight">Pick the plan that fits your goal</h2>
        <p className="mt-4 text-muted-foreground">Cancel or switch any time. Most clients see visible change inside 6 weeks.</p>
      </Reveal>
              ...
              <ul className="mt-6 space-y-3">
                {p.features.map((f) => (
                  <li key={f} className="flex items-start gap-2 text-sm">
                    <Check className={`h-5 w-5 mt-0.5 ${p.popular ? "text-brand" : "text-brand-dark"}`} />
                    <span>{f}</span>
                  </li>
                ))}
              </ul>
